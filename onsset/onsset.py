...
class SettlementProcessor:
    ...
    def calculate_total_demand_per_settlement(self, year, time_step):
        ...
        self.df.loc[self.df[SET_ELEC_FINAL_CODE + "{}".format(year - time_step)] != 99,
                    SET_ENERGY_PER_CELL + "{}".format(year)] += self.df[SET_AGRI_DEMAND] + self.df[
            SET_COMMERCIAL_DEMAND] + self.df[SET_HEALTH_DEMAND] + self.df[SET_EDU_DEMAND]

        self.df[SET_TOTAL_ENERGY_PER_CELL] += self.df[SET_AGRI_DEMAND] + self.df[SET_COMMERCIAL_DEMAND] + self.df[
            SET_HEALTH_DEMAND] + self.df[SET_EDU_DEMAND]
