...
def scenario(specs_path, calibrated_csv_path, results_folder, summary_folder):
    ...
    for scenario in scenarios:
        ...
        all_techs = ["Grid", "SA_Diesel", "SA_PV", "MG_Diesel", "MG_PV", "MG_Wind", "MG_Hydro", "Unelectrified"]
        tech_codes = [1, 2, 3, 4, 5, 6, 7]
        all_tech_codes = [1, 2, 3, 4, 5, 6, 7, 99]
        ...
        for year in yearsofanalysis:
            ...
            num_people_per_hh_rural = float(specs_data.iloc[year][SPE_NUM_PEOPLE_PER_HH_RURAL])
            num_people_per_hh_urban = float(specs_data.iloc[year][SPE_NUM_PEOPLE_PER_HH_URBAN])
            max_grid_extension_dist = float(specs_data.iloc[year][SPE_MAX_GRID_EXTENSION_DIST])
            ...
            grid_calc = Technology(om_of_td_lines=0.02,
                                   distribution_losses=float(specs_data.iloc[year][SPE_GRID_LOSSES]),
                                   connection_cost_per_hh=125,
                                   base_to_peak_load_ratio=0.8,
                                   capacity_factor=1,
                                   tech_life=30,
                                   grid_capacity_investment=float(specs_data.iloc[year][SPE_GRID_CAPACITY_INVESTMENT]),
            ...
            onsseter.calc_summaries(df_summary, sumtechs, all_tech_codes, year, time_step, yearsofanalysis[-1])
